...
> 一句话：**AutoVote 是一家「披着消费级新闻流外衣的预测市场公司」。** 现阶段打的是用户与心智，商业化是**预留的接口**，不是当下要建的引擎。
...
冲突不在「能否共存于一个流」（能，打标签即可），而在三处：①那个数字的**意义相反**（认知准确 vs 注意力诱饵）；②**覆盖面与可信度成反比**（要本地/长尾内容就得制造盘口，违背 A 的可信度）；③**不对称性 —— B 能诚实地容纳 A，A 却无法在不误导用户的前提下容纳 B**。叠加我们用的是**虚拟积分**（用户并未真正参与真钱市场，Polymarket 赔率只作「高质量内容」展示），方向其实早已偏向 B。
